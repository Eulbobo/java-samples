...
public class LogAroundEverything {
    ...
    /**
     * Dans cette déclaration d'aspect, on utilise le nom du pointcut défini plus haut comme régle pour intercepter les
     * appels.
     * Avec le tag @Before, toutes les méthodes correspondant à l'expression du @PointCut insideNorsys() seront pris.
     *
     * Donc tout ce qui est é l'intérieur du package fr.norsys.* et déclaré dans Spring sera intercepté ici
     *
     * Ici, la méthode est statique et utilise un PointCut qui ne l'est pas sans aucun problème.
     * La méthode est privée
     *
     * @param joinPoint le JoinPoint correspondant é l'expression
     * @throws Throwable
     */
    @Before("insideNorsys()")
     ...
     * Pas de paramètre, donc dans ce cas, on n'aura pas d'information sur la méthode d'où on vient de sortir
     */
    @After("insideNorsys()")
